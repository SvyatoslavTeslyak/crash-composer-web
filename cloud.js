...
function render(){
 ...
 (isReviewer()&&pending.length?'<h3>Sent versions</h3><div class="cloud-list">'+pending.map(v=>'<article><strong>'+esc(v.summary)+'</strong>'+authorLine('Sent by',v.submitted_by,v.submitted_at)+'<button class="wb-button" data-view="'+esc(v.id)+'">View sent changes</button> '+releaseActions(v)+'</article>').join('')+'</div>':'')+
