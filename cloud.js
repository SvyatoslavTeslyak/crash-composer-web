...
function reviewVersions(){
 const groups=[['submitted','Needs review','Apply locally accepts this version. Return sends it back for changes.'],['approved','Accepted · not published','Already accepted. Apply locally if needed, then commit and push to publish.']];
 return groups.map(([status,title,hint])=>{const items=sentVersions().filter(v=>v.status===status);return items.length?'<section class="review-queue" data-review-status="'+status+'"><h3>'+title+' <span class="review-count">'+items.length+'</span></h3><p class="review-intro">'+hint+'</p><div class="cloud-list">'+items.map(v=>'<article><strong>'+esc(v.summary)+'</strong>'+authorLine('Sent by',v.submitted_by,v.submitted_at)+'<button class="wb-button" data-view="'+esc(v.id)+'">View sent changes</button> '+releaseActions(v)+'</article>').join('')+'</div></section>':''}).join('')+(published?'<p class="review-published">Published: revision '+esc(published.revision)+' — already live.</p>':'');
}
