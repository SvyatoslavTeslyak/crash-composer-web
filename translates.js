...
function usageMarkup(entry){
 ...
 return '<div class="translation-usage"><span>'+esc(description)+'</span>'+(entry.previewWindow?'<span>Window · '+esc(windowNames[entry.previewWindow]||entry.previewWindow)+'</span>':'')+(entry.presets?'<span>Preset · '+entry.presets.map(p=>p==='standard'?'Standard':'Tabbed shell').join(', ')+'</span>':'')+(entry.previewState?'<span>State · '+(entry.previewState==='API difficulty'?'API difficulty':entry.previewState==='ready'?'Ready':entry.previewState==='win-transfer'?'Win transfer':entry.previewState==='notification'?'Notification':'Round-dependent')+'</span>':'')+'</div>'+(onlyAttribute?'<p class="translation-usage-note">Not a visible caption. This label describes a control to screen readers; the control is outlined in the preview.</p>':'');
}
...
function rows(){const d=current();if(!d)return;typeNavigation();
 ...
 report.innerHTML=importMarkup()+'<div class="translations-heading"><div><span class="translation-eyebrow">TRANSLATES / '+(area==='scene'?'SCENE':'INTERFACE')+'</span><h2>'+title+'</h2><p>'+description+'</p></div><span class="translation-total">'+entries.length+' texts</span></div>'+
