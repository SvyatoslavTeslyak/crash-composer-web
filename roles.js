...
if(!root){
 ...
 function matrix(){return `
  <p class="access-description">Permissions by role. Game assignments determine which games each person can access.</p>
  ${editing?'<form id="role-create" class="role-create"><label>New role<input name="name" maxlength="60" placeholder="Role name" required></label><button type="submit" class="wb-button">Create role</button></form>':''}
  <div class="permission-table-wrap"><table class="permission-table"><caption class="visually-hidden">Permission matrix</caption><thead><tr><th scope="col">Permission</th>${roles.map(role=>`<th scope="col">${editing?`<input aria-label="Role name: ${esc(role.name)}" data-role-name="${esc(role.id)}" value="${esc(role.name)}" maxlength="60" ${role.id==='admin'?'disabled':''}>`:`<strong>${esc(role.name)}</strong>`}<small>${members.filter(m=>m.role===role.id).length} users${role.id==='admin'?' · Protected':''}</small></th>`).join('')}</tr></thead>
  <tbody>${permissions.map(permission=>`<tr><th scope="row">${esc(permission.label)}<small>${esc(permission.section)}${permission.admin_only?' · Admin only':''}</small></th>${roles.map(role=>`<td>${editing?`<input type="checkbox" aria-label="${esc(role.name)}: ${esc(permission.label)}" data-role="${esc(role.id)}" data-permission="${esc(permission.id)}" ${role.permissions.includes(permission.id)?'checked':''} ${role.id==='admin'||permission.admin_only?'disabled':''}>`:`<span class="permission-mark ${role.permissions.includes(permission.id)?'allowed':''}" aria-label="${role.permissions.includes(permission.id)?'Allowed':'Not allowed'}">${role.permissions.includes(permission.id)?'✓':'—'}</span>`}</td>`).join('')}</tr>`).join('')}</tbody>
  ${editing?`<tfoot><tr><th>Save each changed role</th>${roles.map(role=>`<td>${role.id==='admin'?'<span class="access-muted">Protected</span>':`<button class="wb-button" data-save-role="${esc(role.id)}">Save</button><button class="wb-button danger" data-delete-role="${esc(role.id)}" ${members.some(m=>m.role===role.id)?'disabled title="Assign another role to these users first"':''}>Delete</button>`}</td>`).join('')}</tr></tfoot>`:''}</table></div>
  <p class="access-note">Admin has full access. Only Admin can manage roles and people. Brand and shared sound edits apply to the shared library.</p>`}
 function people(){if(!auth.has('users.manage'))return `<p class="access-description">Invite people to the games you can access. Only Admin can view and manage the full member list.</p><button class="wb-button primary" type="button" data-new-invite>Invite person</button>`;return `
  <div class="people-toolbar"><p class="access-description">${members.filter(m=>status(m)!=='removed').length} people · ${members.filter(m=>status(m)==='pending').length} pending invitations</p>${auth.has('users.invite')?'<button class="wb-button primary" type="button" data-new-invite>Invite person</button>':''}</div>
  <div class="permission-table-wrap"><table class="people-table"><caption class="visually-hidden">People and invitations</caption><thead><tr><th scope="col">Email</th><th scope="col">Role</th><th scope="col">Status</th><th scope="col">Invited by</th><th scope="col">Invited on</th>${canEdit()?'<th scope="col">Actions</th>':''}</tr></thead>
  <tbody>${members.map(member=>`<tr data-person="${esc(member.user_id)}"><th scope="row">${esc(member.email)}${member.user_id===auth.session?.user.id?'<small>You</small>':''}</th><td>${esc(roles.find(r=>r.id===member.role)?.name||'—')}</td><td><span class="member-status status-${esc(status(member))}">${esc(statusNames[status(member)]||'Active')}</span></td><td>${esc(member.invited_by_email||(member.invited_at?'Deleted account':'—'))}</td><td><time title="${esc(member.invited_at?new Date(member.invited_at).toLocaleString():'Existing account')}">${esc(date(member.invited_at))}</time></td>${canEdit()?`<td><button class="wb-button" data-edit-member="${esc(member.user_id)}">${status(member)==='removed'?'Restore':'Edit'}</button></td>`:''}</tr>`).join('')||`<tr><td colspan="${canEdit()?6:5}">No people yet.</td></tr>`}</tbody></table></div>
 ...
 function render(){
  root.innerHTML=`<a id="access-back" class="access-back" href="index.html">← Back to Composer</a>
   <header class="access-header"><div><small>WORKSPACE SETTINGS</small><h1>${tab==='roles'?'Roles &amp; permissions':'People &amp; invitations'}</h1></div>${tab==='roles'?`<span class="access-mode">${editing?'Edit mode':'View mode'}</span>`:''}</header>
   <div class="access-controls"><div class="access-actions"><button class="wb-button" data-refresh>Refresh</button>${tab==='roles'&&canEdit()?`<button class="wb-button primary" id="access-edit">${editing?'Done':'Edit'}</button>`:''}</div></div>
  ...
  const editButton=root.querySelector('#access-edit');if(editButton)editButton.onclick=()=>{if(busy||!discard())return;editing=!editing;render();root.querySelector('#access-edit').focus()};
  root.querySelector('[data-refresh]').onclick=()=>{if(discard())run(load)};
  const create=root.querySelector('#role-create');if(create)create.onsubmit=event=>{event.preventDefault();if(roleDirty()){message('Save changed roles before creating another role.');return}const name=new FormData(create).get('name');run(async()=>{await save(null,name,['workspace.view'],null);await load();message('Role created. Choose its permissions and save.')})};
  for(const button of root.querySelectorAll('[data-save-role]'))button.onclick=()=>run(async()=>{
   ...
   const saved=await save(role.id,name,selected,role.revision);roles=roles.map(r=>r.id===saved.id?saved:r);await auth.refreshPermissions();message(saved.name+' saved. Permissions are active.');
  });
