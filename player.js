...
function login(message){session=null;$('game-stage').hidden=true;$('game').hidden=true;$('game').removeAttribute('src');$('login').hidden=false;$('credentials').hidden=false;$('credentials').reset();$('status').textContent=message;controls()}
...
$('credentials').onsubmit=event=>{event.preventDefault();run(async()=>{
 ...
 session={auth:token};const url=new URL(config.gameUrl);url.searchParams.set('api','1');
